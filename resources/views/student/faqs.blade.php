...
<div class="max-w-6xl mx-auto">
                    ...
                    @foreach($categoryFaqs as $index => $faq)
                        <div class="border border-gray-200 rounded-lg">
                            <button class="w-full px-4 py-3 text-left focus:outline-none focus:bg-gray-50 flex justify-between items-center" onclick="toggleFAQ('{{ $category }}-{{ $index }}')">
                                ...
                                <svg class="w-4 h-4 text-blue-500 transform transition-transform" id="icon-{{ $category }}-{{ $index }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- No results message -->
    <div id="no-results" class="bg-white rounded-lg shadow-lg p-6 text-center hidden">
        <h3 class="text-xl font-semibold text-gray-800 mb-2">No results found</h3>
        <p class="text-gray-600">Try adjusting your search terms or browse the categories above.</p>
    </div>
